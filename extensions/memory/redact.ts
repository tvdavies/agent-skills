 ...
 *
 * The brain is a git-tracked store and the ingestion scope is "ALL local sessions",
 * so any miss is committed to git AND later injected into prompts. @jeffs-brain does
 * NO scrubbing; this is the only defence. It deliberately favours OVER-redaction — a
 * memory missing a value is fine; a memory containing a live credential is not.
 *
 * Two layers:
...
const REDACTORS: Array<[RegExp, string]> = [
	...
	[new RegExp(`\\b(?:sk|rk|pk)[_-][A-Za-z0-9_-]{16,}\\b`, "g"), "[REDACTED-TOKEN]"],
	...
	[
		new RegExp(`(^|[^A-Za-z0-9_])((?:[A-Za-z0-9]+[_-])*(?:password|passwd|passphrase|pwd|secret|client[_-]?secret|account[_-]?key|shared[_-]?access[_-]?key))(["']?\\s*[=:]\\s*["']?)(${VALUE}{4,})`, "gi"),
		"$1$2$3[REDACTED]",
	],
	...
	// name like AWS_SECRET_ACCESS_KEY has no internal \\b and the old rule leaked it.
	[
		new RegExp(`(^|[^A-Za-z0-9_])((?:[A-Za-z0-9]+[_-])+(?:api[_-]?key|access[_-]?key|access[_-]?token|auth[_-]?token|refresh[_-]?token|session[_-]?token|private[_-]?key|secret[_-]?key|signing[_-]?key|encryption[_-]?key|key|token|secret))(["']?\\s*[=:]\\s*["']?)(${VALUE}{6,})`, "gi"),
		"$1$2$3[REDACTED]",
	],
	// Bare api_key / apikey assignments (no prefix needed — unambiguous).
	[new RegExp(`(^|[^A-Za-z0-9_])(api[_-]?key|apikey|access[_-]?token|auth[_-]?token)(["']?\\s*[=:]\\s*["']?)(${VALUE}{6,})`, "gi"), "$1$2$3[REDACTED]"],
...
export function redact(text: string): string {
	if (!text) return text;
	return REDACTORS.reduce((acc, [re, rep]) => acc.replace(re, rep as string), text);
}
